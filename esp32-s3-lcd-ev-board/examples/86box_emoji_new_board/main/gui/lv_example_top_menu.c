...
static void main_layer_timer_cb(lv_timer_t *tmr)
{
    ...
    if (pdPASS == app_lvgl_get_event(&lvgl_event, 0)) {
        ...
        case LV_EVENT_LIGHT_ON:
            set_tips_info(sys_set->sr_lang ? "打开电灯" : "Turn On the Light");
            light_set_onoff(true);
            break;
        case LV_EVENT_LIGHT_OFF:
            set_tips_info(sys_set->sr_lang ? "关闭电灯" : "Turn Off the Light");
        ...
        case LV_EVENT_AC_SET_ON:
            ac_set_onoff(true);
            set_tips_info(sys_set->sr_lang ? "打开空调" : "Open the air conditioner");
            break;
        case LV_EVENT_AC_SET_OFF:
            ac_set_onoff(false);
            set_tips_info(sys_set->sr_lang ? "关闭空调" : "Close the air conditioner");
            break;
        case LV_EVENT_AC_TEMP_ADD:
            ac_set_temp(true);
            set_tips_info(sys_set->sr_lang ? "升高温度" : "Raise the temperature");
            break;
        case LV_EVENT_AC_TEMP_DEC:
            ac_set_temp(false);
            set_tips_info(sys_set->sr_lang ? "降低温度" : "Reduce the temperature");
            break;
