...
{
    BSP_ERROR_CHECK_RETURN_ERR(gpio_set_level(BSP_POWER_AMP_IO, (uint8_t)enable));
    return ESP_OK;
}
...
static void bsp_touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (touchpad_pressed && touchpad_cnt > 0) {
        ...
        touch_event = LV_INDEV_STATE_PRESSED;
        ESP_LOGI(TAG, "Touch[%d, %d]: %d,%d", touchpad_pressed, touchpad_cnt, touchpad_x, touchpad_y);
    } else {
        if(LV_INDEV_STATE_PRESSED == touch_event){
            ESP_LOGI(TAG, "Release[%d, %d]: %d,%d", touchpad_pressed, touchpad_cnt, data->point.x, data->point.y);
        }
        touch_event = LV_INDEV_STATE_RELEASED;
